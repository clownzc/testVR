...
{
    specColor = lerp (0.1, albedo, metallic);//specColor = lerp (unity_ColorSpaceDielectricSpec.rgb, albedo, metallic);
...
half3 ShadeSHPerPixel (half3 normal, half3 ambient, float3 worldPos)
{
    half3 ambient_contrib = 0.0;
        ...
        #else
			//SH9不会出现颜色色斑 并且支持法线效果
			ambient_contrib = max(half3(0, 0, 0), ShadeSH9(half4(normal, 1.0)));
            //ambient_contrib = SHEvalLinearL0L1 (half4(normal, 1.0));
        #endif
		
		//0.3184 = 1/pi
        ambient = max(half3(0.00, 0.00, 0.00), ambient + ambient_contrib);     // include L2 contribution in vertex shader before clamp.
        //#ifdef UNITY_COLORSPACE_GAMMA
