...
public class ShatterTool : MonoBehaviour
{
	...
	private void CreateNewGameObjects(IList<IHull> newHulls, out GameObject[] newGameObjects)
	{
		...
		for (int i = 0; i < newHulls.Count; i++)
		{
			...
			{
				if (m_rigidbody == null) m_rigidbody = gameObject.GetComponent<Rigidbody>();
				newRigidbody.mass = m_rigidbody.mass * (volume / totalVolume);
				
				if (!newRigidbody.isKinematic)
				{
					newRigidbody.velocity = m_rigidbody.GetPointVelocity(newRigidbody.worldCenterOfMass);
					
					newRigidbody.angularVelocity = m_rigidbody.angularVelocity;
				}
			}
