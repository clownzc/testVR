...
public class LegacyHull : IHull
{
	...
	private IList<ShatterPoint> vertexToPointMap;
	
	private IList<ShatterPoint> points;
	...
	public LegacyHull(Mesh mesh)
	{
		...
		vertexToPointMap = new List<ShatterPoint>(vertices.Count);
		
		points = new List<ShatterPoint>();
		edges = new List<Edge>();
	...
	public LegacyHull(LegacyHull reference)
	{
		...
		vertexToPointMap = new List<ShatterPoint>(vertexCount);
		
		points = new List<ShatterPoint>(reference.points.Count * 2);
		edges = new List<Edge>(reference.edges.Count * 2);
		triangles = new List<Triangle>(reference.triangles.Count * 2);
	}
	
	private void AddUniquePoint(Vector3 position, out ShatterPoint uniquePoint)
	{
		// Is there already a point with this position?
		foreach (ShatterPoint point in points)
		{
		...
		uniquePoint = new ShatterPoint(position);
		
		points.Add(uniquePoint);
	}
	
	private void AddUniqueEdge(ShatterPoint point0, ShatterPoint point1, out Edge uniqueEdge)
	{
	...
	{
		ShatterPoint point0 = vertexToPointMap[vertex0];
		ShatterPoint point1 = vertexToPointMap[vertex1];
		ShatterPoint point2 = vertexToPointMap[vertex2];
	...
	private void AddVertex(Vector3 vertex, Vector3 normal, Vector4 tangent, Vector2 uv, ShatterPoint point, out int index)
	{
	...
	private void AssignVertices(LegacyHull a, LegacyHull b, bool[] pointAbovePlane, out int[] oldToNewVertex)
	{
		...
		for (int i = 0; i < vertices.Count; i++)
		{
			ShatterPoint correspondingPoint = vertexToPointMap[i];
	...
	private void AssignEdges(LegacyHull a, LegacyHull b, bool[] pointAbovePlane, Vector3 pointOnPlane, Vector3 planeNormal, out bool[] edgeIntersectsPlane, out EdgeHit[] edgeHits)
	{
		...
		foreach (Edge edge in edges)
		{
			...
			else
			{
				...
				// Create new points
				ShatterPoint pointA = new ShatterPoint(intersection);
				ShatterPoint pointB = new ShatterPoint(intersection);
